...
def projection_simplex_sort(v, z=1):
	# EdwardRaff/projection_simplex.py
...
def full_experiment(runs,iters,eta,T,samples):
    ...
    #plt.show()
    plt.savefig('individual_runs__sgd_n{}.png'.format(N),bbox_inches='tight')
    plt.close()
    ...
    # plt.gca().set(xlabel='Iterations',ylabel='L1-accuracy', title='Policy Gradient: agents = {}, runs = {}, $\eta$ = {}'.format(N, runs,eta))
    # #plt.show()
    # plt.savefig('avg_runs_n_sgd{}.png'.format(N),bbox_inches='tight')
    # plt.close()
    ...
    plt.legend()
    plt.savefig('facilities__sgd_n{}.png'.format(N),bbox_inches='tight')
    plt.close()
...
full_experiment(5,1000,0.0001,20,10)
